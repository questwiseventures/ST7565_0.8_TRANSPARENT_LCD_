...
ST7565::ST7565(int8_t SID, int8_t SCLK, int8_t A0, int8_t RST) : Adafruit_GFX(LCDWIDTH, LCDHEIGHT) {
  sid = SID;
  sclk = SCLK;
  a0 = A0;
  rst = RST;
...
// hardware SPI
ST7565::ST7565(int8_t A0, int8_t RST, int8_t CS) : Adafruit_GFX(LCDWIDTH, LCDHEIGHT) {
...
void ST7565::scrollRight(uint8_t s) {
  s &= 0x7F;
  while (s--) {
...
void ST7565::scrollLeft(uint8_t s) {
  s &= 0x7F;
  while (s--) {
    for(uint8_t p = 0; p < 8; p++) {
...
void ST7565::scrollDown(uint8_t s) {
  s &= 0x3F;
  while (s--) {
...
void ST7565::scrollUp(uint8_t s) {
  s &= 0x3F;
  while (s--) {
    for(uint8_t col = 0; col < LCDWIDTH; col++) {
